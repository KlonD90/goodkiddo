...
- Bot database schema changes belong in dialect-specific dbmate SQL migrations under `bot/db/migrations/{sqlite,postgres}/`. Use repo-root Bun scripts `bun run db:migrate`, `bun run db:status`, `bun run db:rollback`, and `bun run db:new -- <name>`; bot startup runs migrations before constructing stores.
