...
- Proactive nudge defaults live in `bot/src/memory/user_profile.ts` as `DEFAULT_PROACTIVE_PREFERENCES`; durable structured values can be parsed/upserted in `USER.md`, and `bot/src/capabilities/proactive/fatigue.ts` returns send/batch/suppress decisions based on quiet hours, timezone availability, digest time, max daily nudges, less-like-this feedback, and explicit user-requested timer/reminder exemptions.
