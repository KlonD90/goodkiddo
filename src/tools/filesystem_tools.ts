...
type RawReadableBackend = BackendProtocol & {
...
function hasReadRaw(backend: BackendProtocol): backend is RawReadableBackend {
  return "readRaw" in backend && typeof backend.readRaw === "function";
}

function hasDownloadFiles(backend: BackendProtocol): backend is DownloadableBackend {
...
function isMultimodalMimeType(mimeType: string | null): boolean {
  return mimeType?.startsWith("image/") === true || mimeType === "application/pdf";
}

// respect https://github.com/langchain-ai/deepagentsjs/blob/4376d51710ef44a446511d873fa6a734a3a42574/libs/deepagents/src/middleware/fs.ts for inspiration
...
export function createReadFileTool(backend: BackendProtocol) {
  return tool(
    async ({
      ...
      const resolvedLimit = limit ?? DEFAULT_READ_LINE_LIMIT;
      const mimeType = detectMimeType(resolvedPath);

      if (isMultimodalMimeType(mimeType) && hasDownloadFiles(backend)) {
      ...
      return await backend.read(resolvedPath, resolvedOffset, resolvedLimit);
    },
    {
      name: "read_file",
