...
const ONE_BY_ONE_PNG = Uint8Array.from([
  ...
  3, 3, 2, 0, 239, 239, 170, 119, 0, 0, 0, 0, 73, 69, 78, 68,
  174, 66, 96, 130,
]);
...
describe("createReadFileTool", () => {
  ...
  test("supports pagination", async () => {
    ...
    const result = await tool.invoke({ file_path: "/notes.txt", offset: 1, limit: 1 });
...
describe("createWriteFileTool", () => {
  ...
  test("returns backend error when writing an existing file", async () => {
    ...
    const result = await tool.invoke({ file_path: "/notes.txt", content: "beta" });
...
describe("createEditFileTool", () => {
  test("edits a file and returns upstream success message", async () => {
    ...
    expect(result).toBe("Successfully replaced 1 occurrence(s) in '/notes.txt'");
    expect(backend.read("/notes.txt", 0, 10)).toBe("     1\talpha\n     2\tgamma");
  });
  ...
  test("supports replace_all", async () => {
    ...
    expect(result).toBe("Successfully replaced 2 occurrence(s) in '/notes.txt'");
    expect(backend.read("/notes.txt", 0, 10)).toBe("     1\tbeta\n     2\tbeta");
  });
});
...
describe("createGrepTool", () => {
  test("groups regex matches by file", async () => {
    ...
    const result = await tool.invoke({ pattern: "brav.|beta", path: "/", glob: "*.ts" });
  ...
  test("returns upstream empty-result message", async () => {
    ...
    const result = await tool.invoke({ pattern: "zeta", path: "/", glob: "*.ts" });
