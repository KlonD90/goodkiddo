...
- [x] **Create `sql_saver.ts` shell with DDL setup** — class scaffold and table creation
...
- [x] **Implement `getTuple` and `readPendingWrites`**
...
- [x] **Implement `list`, `put`, `putWrites`, and `deleteThread`**
  - **Files:** `src/checkpoints/sql_saver.ts`
...
- [x] **Wire in `SqlSaver`, delete `bun_sqlite_saver.ts`**
...
- [x] **Update all instantiation sites to use the shared `SQL` instance**
