...
describe("browser tools + BrowserSessionManager integration", () => {
	test("snapshot at cap returns error without calling CLI", async () => {
		const calls: string[][] = [];
		const run = makeRunner(
			{ open: ok(""), snapshot: ok("content"), get: ok("https://x/") },
			calls,
		);
		const registry = createSessionRegistry("u");
		const manager = createBrowserSessionManager({ maxConcurrent: 1 });
		manager.register("existing", "u-existing");

		const tool = createBrowserSnapshotTool({ run, registry, manager });
		const result = await callTool(tool, { url: "https://x/" });

		expect(result).toContain("error: browser session limit reached");
		expect(calls).toHaveLength(0);
	});

	test("snapshot with room registers with manager and touches after success", async () => {
		const run = makeRunner({
			open: ok(""),
			snapshot: ok("content"),
			get: ok("https://x/"),
	...
	test("snapshot on reaped session key returns expired error", async () => {
		const run = makeRunner({
			snapshot: ok("content"),
			get: ok("https://x/"),
		});
		...
		const result = await callTool(tool, { sessionKey: key });
		expect(result).toContain("error: sessionKey");
		expect(result).toContain("expired due to inactivity");
	});

	test("action on reaped session key returns expired error without CLI call", async () => {
		const calls: string[][] = [];
		const run = makeRunner(
			{ snapshot: ok(""), get: ok("https://x/"), click: ok(""), wait: ok("") },
			calls,
		);
		const registry = createSessionRegistry("u");
		const manager = createBrowserSessionManager({ maxConcurrent: 3 });
	...
	test("successful action touches the manager", async () => {
		const run = makeRunner({
			snapshot: ok(""),
			get: ok("https://x/"),
			click: ok(""),
