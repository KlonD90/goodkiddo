...
const readFileTool = () =>
	tool(
		async (input: { file_path: string }) => `file content: ${input.file_path}`,
		{
			name: "read_file",
...
describe("status ephemerality", () => {
	test("status messages do not appear in tool results - verified by guard wrapping architecture", async () => {
	...
	test("multiple tool calls emit multiple status messages but each tool result contains only actual output", async () => {
		const emitter = new FakeStatusEmitter();
		const broker = new FakeBroker("approve-once");
		...
		const lsResult = await (
			wrappedLs.invoke as (i: unknown) => Promise<unknown>
		)({
			path: "/src",
		});
		const readResult = await (
			wrappedRead.invoke as (i: unknown) => Promise<unknown>
		)({
			file_path: "/src/index.ts",
		});
		const writeResult = await (
			wrappedWrite.invoke as (i: unknown) => Promise<unknown>
		)({
			file_path: "/src/index.ts",
			content: "hello world",
		});
	...
	test("status emitter failures do not affect tool results", async () => {
		const emitter = new FakeStatusEmitter();
	...
	test("architecture: status goes to emitter, not to tool return value - invariant: full_history excludes status", async () => {
		const emitter = new FakeStatusEmitter();
		const broker = new FakeBroker("approve-once");
		const wrapped = wrapToolWithGuard(readFileTool(), {
			caller,
	...
	test("invariant: runtime_context excludes status messages because they are not part of agent state", async () => {
		const emitter = new FakeStatusEmitter();
		const broker = new FakeBroker("approve-once");
		const wrapped = wrapToolWithGuard(readFileTool(), {
			caller,
	...
	test("turn with many tool calls produces many status messages but stored assistant output is just the reply", async () => {
		const emitter = new FakeStatusEmitter();
		const broker = new FakeBroker("approve-once");
...
describe("forced-checkpoints vs runtime_context isolation", () => {
	test("full_history (SqlSaver) and runtime_context are separate stores - status never enters either", async () => {
		const emitter = new FakeStatusEmitter();
		const broker = new FakeBroker("approve-once");
		const wrapped = wrapToolWithGuard(readFileTool(), {
			caller,
		...
		expect(
			sqlSaverStoredMessages.some((m) =>
				JSON.stringify(m).includes(statusMessage),
			),
		).toBe(false);
		expect(
			runtimeContextMessages.some((m) =>
				JSON.stringify(m).includes(statusMessage),
			),
		).toBe(false);
	});

	test("checkpoint compaction does not include status messages in summary", async () => {
		const emitter = new FakeStatusEmitter();
		const broker = new FakeBroker("approve-once");

		const tools = [readFileTool(), writeFileTool(), lsTool()];
