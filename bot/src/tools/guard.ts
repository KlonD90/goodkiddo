	...
	const handler = async (input: unknown): Promise<unknown> => {
		try {
			...
			if (resolved.decision === "deny") {
				log.debug("tool call denied by policy", {
					toolName: original.name,
					callerId: context.caller.id,
			...
			if (resolved.decision === "ask") {
				log.debug("tool call awaiting approval", {
					toolName: original.name,
					callerId: context.caller.id,
					ruleId: resolved.ruleId,
				});
				...
				if (!outcomeApproves(outcome)) {
					log.debug("tool call denied by user", {
						toolName: original.name,
						callerId: context.caller.id,
						outcome,
					});
				...
				log.debug("tool call approved by user", {
					toolName: original.name,
					callerId: context.caller.id,
					outcome,
				});
			...
			//   outcome: "allowed",
			// });
			log.debug("tool call auto-allowed", {
				toolName: original.name,
				callerId: context.caller.id,
				ruleId: resolved.ruleId,
			});
