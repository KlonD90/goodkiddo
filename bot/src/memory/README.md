...
The injected memory rules tell the assistant to run this recall work before asking the user to repeat themselves. A single high-confidence match can be used directly with a brief source mention, medium-confidence matches or competing high-confidence matches require confirmation, and low-confidence results should be presented as 2-3 candidates or narrowed with one targeted clarification. Low confidence is not permission to invent missing context.
